...
public class MelonDS extends Emulator {

    static {
        System.loadLibrary("melonds");
    }

    public MelonDS(@NonNull Resources resources) {
        super("melonds", EmConfig.fromXml(resources.getXml(R.xml.melonds_config)));
    }
    ...
    public static void registerAsEmulator(@NonNull Resources resources) {
        if (sInstance == null) {
            sInstance = new MelonDS(resources);
        }
