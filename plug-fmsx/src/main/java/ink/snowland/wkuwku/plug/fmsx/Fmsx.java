...
public class Fmsx extends Emulator {

    static {
        System.loadLibrary("fmsx");
    }

    public Fmsx(@NonNull Resources resources) {
        super("fmsx", EmConfig.fromXml(resources.getXml(R.xml.fmsx_config)));
    }
    ...
    protected boolean startGame(@NonNull String path) {
        ...
        if (noError) {
            nativeSetControllerPortDevice(0, RETRO_DEVICE_JOYPAD);
            nativeSetControllerPortDevice(1, RETRO_DEVICE_JOYPAD);
        }
        return noError;
    }
