...
public class VbaNext extends Emulator {

    static {
        System.loadLibrary("vbanext");
    }

    public VbaNext(@NonNull Resources resources) {
        super("vbanext", EmConfig.fromXml(resources.getXml(R.xml.vba_config)));
    }
    ...
    public static void registerAsEmulator(@NonNull Resources resources) {
        if (sInstance == null) {
            sInstance = new VbaNext(resources);
        }
